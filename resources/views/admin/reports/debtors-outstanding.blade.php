    ...
    <div class="report-filter-card">
        <form method="GET" action="{{ route('admin.reports.debtors-outstanding') }}" class="row g-3 align-items-end">
            ...
            <div class="col-lg-auto col-md-12 report-filter-actions">
                ...
                <a href="{{ route('admin.export.excel', ['type' => 'debtors', 'financial_year_id' => $financialYearId, 'date_from' => $dateFrom, 'date_to' => $dateTo, 'overdue_status' => $report['filters']['overdue_status'] ?? 'all', 'age_bucket' => $report['filters']['age_bucket'] ?? 'all']) }}" class="btn btn-outline-success report-btn-export">
                    <i class="bi bi-file-earmark-spreadsheet"></i>Excel
                </a>
                <a href="{{ route('admin.export.debtors-outstanding.pdf', ['financial_year_id' => $financialYearId, 'date_from' => $dateFrom, 'date_to' => $dateTo, 'overdue_status' => $report['filters']['overdue_status'] ?? 'all', 'age_bucket' => $report['filters']['age_bucket'] ?? 'all']) }}" class="btn btn-outline-danger report-btn-export">
        ...
        <div class="report-panel-body report-panel-body--flush">
            ...
            <table class="table report-table table-hover mb-0">
                ...
                <tbody>
                    ...
                    @empty
                    <tr><td colspan="9" class="text-muted text-center py-4">No outstanding receivables</td></tr>
                    @endforelse
                </tbody>
                @if($debtors->count() > 0)
                <tfoot>
